...
async def on_message(message: IncomingMessage, model_wrapper: ONNXModelWrapper, channel: aio_pika.Channel) -> None:
    async with message.process():
        props = message.properties
        request_data = json.loads(message.body.decode())
        message_id = request_data.get("message_id")
        ...
        try:
            result = await process_message(message.body.decode(), model_wrapper)
            response_timestamp = time.time()
...
async def main() -> None:
    ...
    await queue.consume(lambda message: on_message(message, model_wrapper, channel))
